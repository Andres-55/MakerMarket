...
//gets 4 pieces of equipment to recommend based on the category the user is looking at
app.get('/equipment/:id/recommendations', async(req, res) => {
    try {
        const equipment = await db.collection("equipment").findOne({
            _id: new ObjectId(req.params.id)
        });

        if(!equipment) return res.status(400).send("Could not find equipment.");

        const recommendations = await db.collection("equipment").find({
